...
class StrykerCli {
    constructor(argv, program = new commander.Command(), runMutationTest = async (options) => new stryker_1.default(options).runMutationTest()) {
        this.argv = argv;
    ...
    run() {
        const dashboard = {};
        const defaultValues = options_validator_1.defaultOptions();
        this.program
        ...
        Optional location to a JSON or JavaScript config file as the last argument. If it's a JavaScript file, that file should export the config directly.`)
            ...
            .option('-f, --files <allFiles>', 'A comma separated list of globbing expression used for selecting all files needed to run the tests. For a more detailed way of selecting input files, please use a configFile. Example: node_modules/a-lib/**/*.js,src/**/*.js,!src/index.js,a.js,test/**/*.js', list)
            .option('-m, --mutate <filesToMutate>', 'A comma separated list of globbing expression used for selecting the files that should be mutated. Example: src/**/*.js,a.js', list)
            ...
            .option('--maxTestRunnerReuse <n>', 'Restart each forked threads after <n> runs. Not recommended unless you are experiencing memory leaks that you are unable to resolve. (default: 0)', parseInt)
            ...
            .option('--cleanTempDir <true/false>', 'Choose whether or not to clean the temp dir (which is ".stryker-tmp" inside the current working directory by default) after a successful run. The temp dir will never be removed when the run failed for some reason (for debugging purposes).', parseBoolean)
            .parse(this.argv);
        // Earliest opportunity to configure the log level based on the logLevel argument
        const options = this.program.opts();
        logging_1.LogConfigurator.configureMainProcess(options.logLevel);
